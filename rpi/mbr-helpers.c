...
void mbr_check(struct mbr *mbr) {
    // Ammar Ratnani: It's possible that `mbr_read` returns `NULL` - we don't
    // have the source, so we can't be sure. We should check for this.
    assert(mbr);
    ...
    // Ammar Ratnani: We need to disable this check because we are using a
    // second partition for testing.
    //
...
void mbr_partition_print(const char *msg, struct partition_entry *p) {
    ...
    // Ammar Ratnani: We need to disable this check because the test partition
    // is not formatted.
